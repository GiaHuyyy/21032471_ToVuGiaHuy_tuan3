...
export default function Screen_03({ navigation }) {
  ...
  const updateQuantity = (key, delta) => {
    setData((prevData) =>
      prevData.map((item) => (item.key === key ? { ...item, quantity: Math.max(item.quantity + delta, 1) } : item))
    );
  };
        ...
        <View style={styles.totalContainer}>
          <View style={styles.totalRow}>
            ...
            <Text style={styles.totalText}>
              ${data.reduce((total, item) => total + parseFloat(item.price) * item.quantity, 0).toFixed(2)}
            </Text>
          </View>
